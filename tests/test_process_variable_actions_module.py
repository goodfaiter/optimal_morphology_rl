"""Unit tests for the process_variable_actions module."""
...
def _make_module() -> ProcessVariableActionsModule:
    return ProcessVariableActionsModule({
...
def test_step_updates_buffers(container: ModuleContainer) -> None:
    ...
    # Scaling: root DOFs scaled by config velocity, active DOFs by config dof scale.
    expected_root = torch.tensor([1.0, 1.0, 1.0, 0.2, 0.2, 0.2], dtype=torch.float32) * 0.5
    expected_dof = torch.full((2,), 0.1, dtype=torch.float32) * 0.5
