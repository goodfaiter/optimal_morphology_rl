"""Unit tests for the process_fixed_actions module."""
...
def _make_module() -> ProcessFixedActionsModule:
    return ProcessFixedActionsModule({
        "min_action_scale": [-1.0, -1.0, -1.0, -0.2, -0.2, -0.2, -0.1, -0.1],
        "max_action_scale": [1.0, 1.0, 1.0, 0.2, 0.2, 0.2, 0.1, 0.1],
    })
...
def test_step_updates_buffers(container: ModuleContainer) -> None:
    ...
    # Scaling: per-action scales from config.
    expected_root = torch.tensor([1.0, 1.0, 1.0, 0.2, 0.2, 0.2], dtype=torch.float32) * 0.5
    expected_dof = torch.tensor([0.1, 0.1], dtype=torch.float32) * 0.5
