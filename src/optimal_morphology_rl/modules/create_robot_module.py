...
class Robot:
    ...
    Read-state buffers are attached by ``update_robot`` during its
    ``post_finalize`` hook.
    """
...
class RobotModule(BaseModule):
    ...
    def reset(self, container: ModuleContainer) -> None:
        ...
        if self.fixed_hand:
            container.reset_root_transform_buf[reset_buf, 4:] = self.fixed_hand_root_pos
            container.reset_root_transform_buf[reset_buf, :4] = self.fixed_hand_root_quat
        else:
            if randomize_pose:
                n_reset = reset_buf.sum().item()
                container.reset_root_transform_buf[reset_buf, :4] = random_uniform_quaternion(n_reset, device=device, dtype=torch.float32)
                container.reset_root_transform_buf[reset_buf, 4] = -0.1
                container.reset_root_transform_buf[reset_buf, 5] = torch.rand(n_reset, device=device) * 0.3 - 0.15
                container.reset_root_transform_buf[reset_buf, 6] = torch.rand(n_reset, device=device) * 0.2 + 0.1
            else:
                container.reset_root_transform_buf[reset_buf, 4:] = torch.tensor([[-0.1, -0.15, 0.2]], device=device)
